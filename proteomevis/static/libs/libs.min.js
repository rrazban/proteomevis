 ...
 * colorbar.min.js
 */
 function Colorbar(){function c(c){var d,e;c.selectAll(".pointer").remove(),c.pointTo=function(a){var c=(d.selectAll(".pointer"),Math.round(3*thickness/4));pointers=d.selectAll(".pointer").data([a]),pointerSVGdef=function(){return"horizontal"==orient?"M 0 "+thickness+" l -"+c+" -"+c+" l "+2*c+" -0 z":"M "+thickness+" 0 l -"+c+" -"+c+" l 0 "+2*c+" z"},pointers.enter().append("path").attr("transform","vertical"==orient?"translate(0,"+e(a)+")":"translate("+e(a)+",0)").classed("pointer",!0).classed("axis",!0).attr("d",pointerSVGdef()).attr("fill","grey").attr("opacity","0"),pointers.transition().duration(1e3).attr("opacity",1).attr("transform","vertical"==orient?"translate(0,"+e(a)+")":"translate("+e(a)+",0)").transition().delay(2e3).duration(3e3).attr("opacity",0).remove()},c.each(function(f){var h,i,j,k,l,m;checkScaleType(a);if("horizontal"===orient){var n=[b.left,b.right,b.top,b.bottom];b.top=n[0],b.bottom=n[1],b.left=n[2],b.right=n[3],h="height",i="width",j="bottom",k="x",l="y",m="translate (0,"+thickness+")"}else h="width",i="height",j="right",k="y",l="x",m="translate ("+thickness+",0)";var o=d3.select(this).selectAll("svg.colorbar").data([origin]),p=o.enter().append("svg").classed("colorbar",!0).attr("x",function(a){return a[0]-b.right}).attr("y",function(a){return a[1]-b.top});offsetGroup=p.append("g").classed("colorbar",!0).attr("transform","translate("+b.left+","+b.top+")"),offsetGroup.append("g").attr("class","legend rectArea"),offsetGroup.append("g").attr("class","axis color"),o.attr(h,thickness+b.left+b.right).attr(i,barlength+b.top+b.bottom).style("margin-top",origin.y-b.top+"px").style("margin-left",origin.x-b.left+"px"),d=o.selectAll("g.colorbar").attr("transform","translate("+b.left+","+b.top+")"),e=a.copy(),"undefined"==typeof e.invert&&(e=d3.scale.linear().domain(d3.extent(e.domain())));var q=d3.range(0,barlength,by=barlength/(e.domain().length-1));q.push(barlength),"vertical"==orient&&q.reverse(),e.range(q),colorScaleRects=d.selectAll("rect.legend").data(d3.range(0,barlength)),colorScaleRects.enter().append("rect").attr("class","legend").style("opacity",0).style("stroke-thickness",0).style("fill",function(b){return a(e.invert(b))}),colorScaleRects.exit().remove(),c.selectAll("rect.legend").style("opacity",1).attr(h,thickness).attr(i,2).attr(k,function(a){return a}).attr(l,0).style("fill",function(b){return a(e.invert(b))}),colorAxisFunction=d3.svg.axis().scale(e).tickFormat(d3.format(".2f")).orient(j),"undefined"!=typeof a.quantiles&&(quantileScaleMarkers=a.quantiles().concat(d3.extent(a.domain())),console.log(quantileScaleMarkers),colorAxisFunction.tickValues(quantileScaleMarkers)),d.selectAll(".color.axis").attr("transform",m).call(colorAxisFunction),titles=d.selectAll(".axis.title").data([{label:title}]).attr("id","#colorSelector").attr("transform","translate (0, -10)").style("text-anchor","middle").text(function(a){return a.label}),titles.exit().remove()})}var a,b={top:5,right:30,bottom:25,left:0};return orient="vertical",origin={x:0,y:0},barlength=100,thickness=50,title="",scaleType="linear",checkScaleType=function(a){return cop=a.copy(),cop.range([0,1]),cop.domain([1,10]),"undefined"!=typeof cop.invertExtent?"quantile":Math.abs((cop(10)-cop(1))/Math.log(10)-(cop(10)-cop(2))/Math.log(5))<1e-6?"log":Math.abs((cop(10)-cop(1))/9-(cop(10)-cop(2))/8)<1e-6?"linear":Math.abs((cop(10)-cop(1))/(Math.sqrt(10)-1)-(cop(10)-cop(2))/(Math.sqrt(10)-Math.sqrt(2)))<1e-6?"sqrt":"unknown"},c.origin=function(a){return arguments.length?(origin=a,c):origin},c.margin=function(a){return arguments.length?(b=a,c):b},c.thickness=function(a){return arguments.length?(thickness=a,c):thickness},c.barlength=function(a){return arguments.length?(barlength=a,c):barlength},c.title=function(a){return arguments.length?(title=a,c):title},c.scale=function(b){return arguments.length?(a=b,c):a},c.orient=function(a){return arguments.length?("vertical"===a||"horizontal"===a?orient=a:console.warn("orient can be only vertical or horizontal, not",a),orient=a,c):orient},c}
